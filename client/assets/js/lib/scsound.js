...
var SCSound = {
	...
	send: function(eventName, _args) {
		if (SCSound.disable || !SCSound.isInited) return;
	...
	trig: function(event, args) {
		...
		for (var i = 0; i < targets.length; i++) {
			...
			case "BypassEventsAction":
				this.bypass = true;
				this.exeptions = ["ready_set_go", "race_loop_1", "race_loop_2", "race_loop_3", "race_loop_4", "race_loop_5", "stop_race_loop", "bridge_android", "fanfare_all", "stop_bridge_loop"];
				break;
	...
	play: function(time, group, soGain, loop, offsetSound, loopStart, loopEnd) {
		...
		if (this.audioSprite) {
			...
			if (SCSound.webaudio) {
				...
				if (SCSound.audioTag.src.indexOf(SCSound.soundPath+ this.name + ".mp3") === -1) {
					console.log("set src");
...
SCSound.Core.Group = function(vol, inserts, sends) {
	...
	this.effects = {};
	if (SCSound.webaudio) {
		this.lastNode = SCSound.Core.SoundController.context.createGainNode();
